...
#     plot!(p, Ts, max_Δ, label=nothing, c=cmap[i], yaxis=:log10)
#     scatter!(p, Ts, max_Δ, label="J=$J", c=cmap[i], yaxis=:log10)

#     # Get the LGE data 
#     dfsub_LGE = df_LGE[(df_LGE.J.==J).&(df_LGE.V0.==V0).&(df_LGE.V1.==V1), :]
#     Tc = find_Tc(dfsub_LGE).Tc

#     vline!(p, [Tc], label="LGE Tc", c=cmap[i])
# end

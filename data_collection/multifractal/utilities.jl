...
function already_computed(df, m::ModelParams; ℓ, E₀, α₀)
    # extract only the parameters we are interested in 
    df = df[(df.L.==m.L).&(df.θ.==m.θ).&(df.Q.==m.Q).&(df.ϕx.==m.ϕx).&(df.ϕy.==m.ϕy).&(df.ϕz.==m.ϕz).&(df.ndims.==m.ndims).&(df.J.==m.J).&(df.ℓ.==m.ℓ).&(df.E₀.==m.E₀).&(df.α₀.==m.α₀), :]
    if size(df)[1] > 0
...
function save_results(m::ModelParams; ℓ, E₀, α₀)

...
function converge_BdG_swave(m::ModelParams; T::Real, Δ_init=nothing, niter::Int=500, tol::Real=1e-12, scratchpath=nothing)
    ...
    if !isnothing(scratchpath)
        df = load_BdG_checkpoint(m, scratchpath)
        if size(df)[1] > 0
            nstart = df.n[1]
            Δ_init = df.Δ[1]
...
function BdG_coefficients(m; T::Real, Δ_init=nothing, niter::Int=100, tol::Union{Real,Nothing}=nothing, scratchpath=nothing)
...
function load_BdG_checkpoint(m::ModelParams, dirname)
    # read files 
